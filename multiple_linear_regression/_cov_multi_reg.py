...
def cov (x,md_x):
    cov_xy=[[0 for r in range(data.shape[1])] for c in range(data.shape[1])]
    for i in range(data.shape[1]):
        for j in range (data.shape[1]):
            for k in range (data.shape[0]):
                cov_xy[i][j]+=((data[i][k]-md_x[i])*(data[j][k]-md_x[j]))/(data.shape[0]-1)
    return(cov_xy)
...
#   col=data.shape[1]
    ...
    mean=avg(data,data.shape[0],data.shape[1])
...
#   i_end=len(b)
    ...
    j_start=0
    print("TEST THE EFFECT OF EACH REGRESSOR")
    print("index of the regressor vector :", i_start,j_start)
    print("")
    ...
    j_end= 2
    c_xx= filter (covar, i_start,i_end, j_start, j_end)
    #print("covariance of the regressor")
    #printing(c_xx)
    # inverse of the matrix with covariances between regressors
